...
try:
    from gui_dialogs import SettingsDialog, AddFilesDialog, BuildSettingsDialog, ProjectDialog
except ImportError:
...
class ZHTLLM_GUI(QMainWindow):
    def __init__(self):
        ...
        self.run_build_btn = QPushButton("Run Build") # Added a button to trigger build
        ...
        self.run_query_btn = QPushButton("Run Query") # Added a button to trigger query
        ...
        # The settings button is not part of the splitter to keep it fixed on the right
    ...
    def open_settings_dialog(self):
        ...
        # TODO: Pass current settings to the dialog
        if dialog.exec():
            self.settings = dialog.get_settings()
            self.output_console.append("Settings updated.")
            # Optionally, you can print the settings to the console for verification
            # import json
            # self.output_console.append(json.dumps(self.settings, indent=2))
    ...
    def open_add_files_dialog(self):
        ...
        if dialog.exec():
            ...
            # import json
            # self.output_console.append(json.dumps(self.build_params, indent=2))

    def open_build_config_dialog(self):
        dialog = BuildSettingsDialog(self, self.settings)
        if dialog.exec():
            config = dialog.get_build_config()
            # Update main settings based on build-specific config
            self.settings['inference_mode'] = config['inference_mode']
            
            if config['inference_mode'] == 'local':
                self.settings['local_model'] = config['model']
                self.output_console.append(f"Build configuration updated: Mode set to local, model set to {config['model']}.")
            
            elif config['inference_mode'] == 'cloud':
                self.settings['cloud_model'] = config['model']
                if 'api_base' in config:
                    self.settings['cloud_api_base'] = config['api_base']
                if 'api_key' in config:
                    self.settings['cloud_api_key'] = config['api_key']
                self.output_console.append(f"Build configuration updated: Mode set to cloud, model set to {config['model']}.")

    def execute_build(self):
        if not self.build_params or (not self.build_params.get('dialogue') and not self.build_params.get('text_files')):
            self.output_console.append("Error: No files selected for build. Use 'add files' first.")
            return
        
        if not self.current_project:
            self.output_console.append("Error: No project selected. Please create or select a project first.")
            return

        self.output_console.append(f"Starting build process for project '{self.current_project}'...")
        self.run_build_btn.setEnabled(False)
        self.run_query_btn.setEnabled(False)

        # Combine build params with the relevant settings for the run_build function
    ...
    def execute_search(self):
        ...
        if not query:
            self.output_console.append("Error: Query is empty.")
            return
        ...
        self.output_console.append(f"Starting search for: '{query}' in project '{self.current_project}'")
        self.run_build_btn.setEnabled(False)
        self.run_query_btn.setEnabled(False)

        search_args = {**self.settings, "query": query, "name": self.current_project}

        # Clear previous search results before running a new one
    ...
    def on_build_finished(self, result):
        ...
        if result:
            ...
            self.output_console.append("Build process completed with no message.")
    ...
    def on_search_finished(self, result):
        self.output_console.append("\n--- Search Finished ---")
        if result:
            # The result from run_search is now the captured stdout
            self.output_console.append(result)
        else:
            self.output_console.append("Search completed with no result.")
        self.run_build_btn.setEnabled(True)
        self.run_query_btn.setEnabled(True)
        self.worker = None
...
if __name__ == "__main__":
    print("--- GUI Environment Diagnostics ---")
